...
src_prepare() {
	...
	# Currently put in this step, will be moved in src_test until code generation works as intended.
	# Source: https://github.com/gentoo/gentoo/pull/27990#issuecomment-1296217239
	if use test; then
		# https://github.com/protocolbuffers/protobuf/blob/v21.9/java/core/pom.xml#L98

		sed \
			-e "s:\${generated.testsources.dir}:${JAVA_TEST_SRC_DIR}:" \
			-e "s:\${protobuf.source.dir}:${JAVA_TEST_RESOURCE_DIRS}:" \
			-e 's:${test.proto.dir}:core/src/test/proto:' \
			-e 's:^.*value="::' \
			-e 's:\"/>::' \
			-e '/project\|mkdir\|exec\|Also generate/d' \
			-i core/generate-test-sources-build.xml || die

		edo "${BROOT}/usr/bin/protoc" @core/generate-test-sources-build.xml
	fi
}
